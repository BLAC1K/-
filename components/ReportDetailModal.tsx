...
const ReportDetailModal: React.FC<ReportDetailModalProps> = ({ report, user, viewerRole, onClose, hideMargin = false }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-0 sm:p-4 md:p-8 no-print" aria-modal="true" role="dialog" onClick={onClose}>
            {/* الحاوية الرئيسية - id="printable-area" هو المرجع الوحيد للطباعة في index.html */}
            <div id="printable-area" className="relative w-full h-full max-w-5xl sm:h-[95vh] sm:rounded-[2.5rem] bg-white dark:bg-gray-800 shadow-2xl flex flex-col overflow-hidden print:!bg-white print:!h-auto print:!overflow-visible" onClick={e => e.stopPropagation()}>
                
                {/* ترويسة الطباعة الرسمية - تظهر في الورق/PDF فقط */}
                <div className="hidden print:block p-10 border-b-4 border-double border-black mb-6">
                ...
                {/* رأس الشاشة الرقمي - يتم إخفاؤه تماماً عند الطباعة */}
                ...
                {/* محتوى التقرير - يتدفق بحرية في الطباعة */}
                <div className="flex-grow overflow-y-auto no-scrollbar bg-white dark:bg-gray-900 print:!h-auto print:!overflow-visible">
